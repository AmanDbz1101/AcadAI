...
from backend.extraction.models.document import ValidatedDocument, PageContent
from backend.extraction.models.metadata import ProcessedDocument, SectionInfo
from backend.extraction.models.section_hierarchy import SectionNode, SectionHierarchy
...
class SectionDetector:
    ...
    def _calculate_page_ranges(self, sections: List[SectionNode]) -> None:
        ...
        Handles cases where multiple sections start on the same page.
        Modifies sections in place.
        """
        # Sort sections by page_start, then by reading_order for stable sorting
        sorted_by_page = sorted(enumerate(sections), 
                               key=lambda x: (x[1].page_start, x[1].reading_order))
        
        for i in range(len(sorted_by_page)):
            original_idx, section = sorted_by_page[i]
            current_page = section.page_start
            
            if i < len(sorted_by_page) - 1:
                # Find the next section that starts on a different page
                next_different_page = None
                for j in range(i + 1, len(sorted_by_page)):
                    next_section = sorted_by_page[j][1]
                    if next_section.page_start > current_page:
                        next_different_page = next_section.page_start
                        break
                
                if next_different_page:
                    # End page is one before next section starts on a different page
                    sections[original_idx].page_end = next_different_page - 1
                else:
                    # No more sections on different pages, ends on current page
                    sections[original_idx].page_end = current_page
            else:
                # Last section: ends on its start page (or None for last in document)
                sections[original_idx].page_end = None
