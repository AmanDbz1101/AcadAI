...
# --- Use new guide generation modules for endpoints ---
@app.get("/guide_generation", response_class=JSONResponse)
def guide_generation():
    """
    Returns the full LLM-based 3-pass reading guide using the detailed schema and prompt.
    """
    base_dir = os.path.dirname(os.path.abspath(__file__))
    metadata_path = os.path.join(base_dir, "output_Gated Attention_metadata.json")
    output_path = os.path.join(base_dir, "output", "Gated_Attention_for_Large_Language_Models:_Non-linearity,_Sparsity,_and_Attention-Sink-Free_reading_guide.json")
    from src.guide_generation.full_guide_generation import generate_full_guide_llm, load_full_guide
    if not os.path.exists(output_path):
        guide = generate_full_guide_llm(metadata_path, output_path)
    else:
        guide = load_full_guide(output_path)
    return JSONResponse(content=guide)

@app.get("/minimal_guide_generation", response_class=JSONResponse)
def minimal_guide_generation():
    """
    Returns the minimal 3-pass reading guide generated by LLM from output_Gated Attention_metadata.json.
    """
    base_dir = os.path.dirname(os.path.abspath(__file__))
    metadata_path = os.path.join(base_dir, "output_Gated Attention_metadata.json")
    output_path = os.path.join(base_dir, "output", "Gated_Attention_for_Large_Language_Models:_Non-linearity,_Sparsity,_and_Attention-Sink-Free_reading_guide_minimal.json")
    from src.guide_generation.minimal_guide_generation import generate_minimal_guide_llm, load_minimal_guide
    if not os.path.exists(output_path):
        guide = generate_minimal_guide_llm(metadata_path, output_path)
    else:
        guide = load_minimal_guide(output_path)
    return JSONResponse(content=guide)
