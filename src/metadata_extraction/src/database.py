...
from src.metadata_extraction.src.models import PaperMetadata, SectionMetadata, PaperInference
from src.metadata_extraction.src.text_extraction import TextBlock


class DatabaseManager:
    """Manages PostgreSQL database for research paper metadata storage."""
    
    def __init__(self, db_url: Optional[str] = None):
        """Initialize database manager.
        
        Args:
            db_url: PostgreSQL DSN/URL. Falls back to POSTGRES_DSN.
        """
    ...
    def store_extraction_results(
        ...
        with self._get_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                cursor.execute("SELECT id FROM papers WHERE pdf_path = %s", (pdf_path,))
                ...
                if existing:
                    ...
                    cursor.execute(
                        """
                        UPDATE papers 
                        SET title = %s, abstract = %s, paper_type = %s, 
                            difficulty = %s, math_heavy = %s, 
                    ...
                    cursor.execute("DELETE FROM text_blocks WHERE paper_id = %s", (paper_id,))
                    cursor.execute("DELETE FROM sections WHERE paper_id = %s", (paper_id,))
                else:
                    # Insert new paper
                ...
                for block in text_blocks:
                    metadata_json = Json(block.metadata) if block.metadata else None
                    cursor.execute(
                        """
    ...
    def get_paper_by_path(self, pdf_path: str) -> Optional[Dict[str, Any]]:
        ...
        with self._get_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                cursor.execute("SELECT * FROM papers WHERE pdf_path = %s", (pdf_path,))
                row = cursor.fetchone()
            ...
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                if page_number is not None:
                    cursor.execute(
                        ...
                        WHERE paper_id = %s AND page_number = %s
                        ORDER BY id
                        """,
                ...
                for row in rows:
                    block = dict(row)
                    # JSONB is already decoded by psycopg2.
                    block["metadata"] = block.get("metadata_json") or {}
        ...
        with self._get_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                if "abstract" in search_in:
                    conditions.append("abstract ILIKE %s")
                    params.append(f"%{query}%")
    ...
    def get_paper_statistics(self) -> Dict[str, Any]:
        ...
        with self._get_connection() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cursor:
                ...
                papers_by_type = {
                    row["paper_type"]: row["count"] for row in cursor.fetchall()
                }
