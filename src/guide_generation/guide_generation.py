...
# Minimal Pydantic models - only essential info for next agent
class ReadingStep(BaseModel):
    """Minimal step info for next agent"""
...
class StepsGeneratorOutput(BaseModel):
    """Minimal reading guide output"""
    paper_id: str
    passes: List[ReadingPass]

def generate_minimal_guide_llm(metadata_path, output_path):
    """
    Generate a minimal 3-pass reading guide using LLM and save as JSON.
    """
    ...
    # Minimal prompt - let next agent figure out details
